...
export function rememberPoll(poll) {
  ...
  const next = [summarizePoll(poll), ...listRememberedPolls().filter((p) => p.id !== poll.id)]
...
export const api = {
  ...
  createPoll: (question, options) =>
    request('/api/polls', { method: 'POST', auth: true, body: { question, options } }),
  getPoll: (id) => request(`/api/polls/${id}`),
  vote: (id, optionId) =>
    request(`/api/polls/${id}/vote`, {
      method: 'POST',
      body: { optionId, voterKey: getUserId() || getVoterKey() },
    }),
}
